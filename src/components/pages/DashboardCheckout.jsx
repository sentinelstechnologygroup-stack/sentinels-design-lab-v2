...
export default function DashboardCheckout({ offer, websites = [] }) {
  ...
  return <>
    <button onClick={() => { setOpen(true); setError(""); }} className="mt-5 inline-flex min-h-10 items-center gap-2 rounded-lg bg-primary px-4 text-sm font-semibold text-white">Review report &amp; access <ArrowRight className="h-4 w-4" /></button>
    {open && <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/75 p-4 backdrop-blur-sm" role="dialog" aria-modal="true" aria-labelledby={`offer-${offer.code}`}>
      <div className="max-h-[90dvh] w-full max-w-4xl overflow-y-auto rounded-2xl border border-white/15 bg-[#08101d] p-5 shadow-2xl sm:p-8">
        ...
        <div className="mt-6 grid gap-3 md:grid-cols-2">{REPORT_CATALOG.filter((report) => fixedSelection.includes(report.code)).map((report) => <div key={report.code} className="flex gap-3 rounded-xl border border-primary/50 bg-primary/[0.08] p-4 text-left"><span className="mt-0.5 flex h-5 w-5 shrink-0 items-center justify-center rounded border border-primary bg-primary text-white"><Check className="h-3.5 w-3.5" /></span><span><strong className="text-sm text-foreground">{report.name}</strong><span className="mt-1 block text-xs leading-5 text-muted-foreground">{report.description}</span><span className="mt-2 block text-xs font-medium text-cyan-200">Required connections: {report.requiredConnections.join(", ").replaceAll("search-console", "Google Search Console").replaceAll("business-profile", "Google Business Profile").replaceAll("analytics", "Google Analytics 4").replaceAll("ads", "Google Ads").replaceAll("tag-manager", "Google Tag Manager")}</span></span></div>)}</div>
