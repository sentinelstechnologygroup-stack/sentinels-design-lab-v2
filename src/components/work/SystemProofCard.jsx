...
function statusBadgeClass(status) {
  if (status === "LIVE SYSTEM")
    return "border-emerald-400/25 bg-emerald-400/[0.08] text-emerald-400";
  if (status === "ACTIVE BUILD")
    return "border-sky-400/25 bg-sky-400/[0.08] text-sky-400";
...
export default function SystemProofCard({
  name,
  whatItDoes = [],
  systemRole = [],
  visualSrc,
      ...
      <div className="p-6 md:p-8">
        <div className="flex flex-wrap items-start justify-between gap-4">
          ...
          <div className="flex flex-wrap gap-2 text-xs">
            <span
              className={`rounded-full border px-3 py-1 ${statusBadgeClass(status)}`}
            >
              {status}
            </span>
        ...
        {confidentialityNote ? (
          <p className="mt-4 rounded-2xl border border-white/10 bg-white/[0.03] px-4 py-4 text-sm leading-7 text-white/65">
            {confidentialityNote}
          </p>
        ) : null}
