...
function scoreRows(categories) {
  return categories
    .map(
      (item) =>
        `<tr><td style="padding:10px 0;color:#334155;font-size:14px">${escape(item.label)}</td><td style="padding:10px 0;text-align:right;font-weight:700;color:#0f172a">${item.score}/100</td></tr>`,
    )
    .join("");
}
...
function unknownRows(items = []) {
  return items
    .map(
      (item) =>
        `<tr><td style="padding:10px 0;color:#334155;font-size:14px"><strong>${escape(item.label)}</strong><br><span style="color:#64748b;font-size:12px">${escape(item.reason)}</span></td><td style="padding:10px 0;text-align:right;color:#b45309;font-size:12px;font-weight:700">${escape(item.status)}</td></tr>`,
    )
    .join("");
}
...
export async function sendEvaluationEmails({
  evaluation,
  lead,
  pdf,
  portalUrl,
  ...
  const admin =
    process.env.SIS_NOTIFICATION_EMAIL || "reports@sentinelsdesignlab.com";
  const attachment = {
    filename: `Sentinels-Design-Lab-Website-Evaluation-${evaluation.businessName.replace(/[^a-z0-9]+/gi, "-")}.pdf`,
  ...
  const adminResult = await sendMail({
    ...
    to: admin,
    replyTo: lead.email,
    subject: `New Sentinels Intelligence Suite lead: ${evaluation.businessName} readiness snapshot`,
