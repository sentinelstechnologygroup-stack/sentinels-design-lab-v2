          ...
          <div className="grid lg:grid-cols-2 gap-10 items-end">
            <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }} className="pb-12 lg:pb-16">
              ...
              <div className="flex flex-wrap gap-3">
                {platforms.map((p, i) => (
                  <div key={i} className="surface-card px-5 py-3 flex items-center gap-3">
                    <div>
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
            {filtered.map((review, i) => (
              <motion.div
                ...
                className="surface-card p-7 flex flex-col hover:border-primary/30 transition-all"
              >
                <div className="flex items-start justify-between mb-4">
                  <Quote className="w-8 h-8 text-amber-300/40" />
                  <div className="flex gap-0.5">
                    {Array.from({ length: review.rating }).map((_, j) => (
                      <Star key={j} className="w-4 h-4 text-amber-300 fill-amber-300" />
                    ))}
                  </div>
                </div>
                ...
                {review.result && (
                  <div className="flex items-center gap-2 bg-primary/10 border border-primary/20 rounded-lg px-3 py-2 mb-5">
                    <ThumbsUp className="w-3.5 h-3.5 text-amber-300 shrink-0" />
                    <span className="text-xs font-semibold text-amber-300">{review.result}</span>
                  </div>
                )}
                ...
                <div className="border-t border-border/40 pt-4 flex items-center justify-between">
                  ...
                  <span className="pill-primary text-xs px-2.5 py-1 rounded-full">{review.service}</span>
                </div>
              </motion.div>
            ))}
          </div>
