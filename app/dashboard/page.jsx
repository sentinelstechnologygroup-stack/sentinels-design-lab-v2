...
export default async function DashboardPage({ searchParams }) {
  ...
  return <main className="min-h-screen pb-24 pt-28"><section className="mx-auto max-w-6xl px-6">
    ...
    <section className="mt-16"><span className="eyebrow mb-4">Advanced Reports</span><div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-end"><div><h2 className="font-heading text-3xl font-bold">Reports and bundles</h2><p className="mt-2 max-w-2xl text-sm leading-6 text-muted-foreground">Move beyond a public-page snapshot with market, search, local, paid-media, reputation and conversion evidence. Choose the exact reports you want before entering checkout.</p></div><ShoppingBag className="h-8 w-8 text-primary" /></div><div className="mt-5 grid gap-4 md:grid-cols-2 lg:grid-cols-3">{Object.values(REPORT_OFFERS).map((offer) => <article key={offer.code} className="rounded-2xl border border-white/10 bg-card p-6"><h3 className="font-semibold">{offer.name}</h3><div className="mt-3 font-heading text-3xl font-bold text-primary">${(offer.amountCents / 100).toFixed(0)}</div><p className="mt-2 text-xs leading-5 text-muted-foreground">{offer.summary || "Choose the report areas that matter most to this business."}</p><p className="mt-3 text-xs leading-5 text-white/45">Hosting credit: {offer.hostingCredit}</p><DashboardCheckout offer={offer} /></article>)}</div><div className="mt-6 rounded-xl border border-white/10 bg-white/[0.025] p-4 text-xs leading-5 text-muted-foreground">{REPORTING_POLICY.changeNotice} Eligible managed service packages also include recurring website report cards so customers can monitor changes over time.</div><Link href="/systems/sis" className="mt-6 inline-flex items-center gap-2 text-sm font-semibold text-primary">Compare all report contents <ArrowRight className="h-4 w-4" /></Link></section>
