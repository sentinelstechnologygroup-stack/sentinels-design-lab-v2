...
export default async function DashboardPage({ searchParams }) {
  ...
  return <main className="min-h-screen pb-24 pt-28"><section className="mx-auto max-w-6xl px-6">
    ...
    <section className="mt-12"><span className="eyebrow mb-4">Saved Reports</span><h2 className="font-heading text-3xl font-bold">Your report library</h2><p className="mt-2 text-sm text-muted-foreground">Reports stay private and available from this account.</p>{savedReports.length ? <div className="mt-5 grid gap-4 md:grid-cols-2">{savedReports.map((report) => <article key={report.id} className={`rounded-2xl border bg-card p-6 ${params?.report === report.id ? "border-primary/50 ring-1 ring-primary/20" : "border-white/10"}`}><div className="flex items-start justify-between gap-4"><FileText className="h-6 w-6 text-primary" /><span className="rounded-full bg-white/[0.06] px-3 py-1 text-xs text-muted-foreground">{report.status}</span></div><h3 className="mt-4 font-semibold">{report.title}</h3><p className="mt-2 text-xs text-muted-foreground">{new Date(report.generatedAt).toLocaleDateString("en-US")}</p>{report.findings?.verdict && <p className="mt-4 text-sm leading-6 text-muted-foreground">{report.findings.verdict}</p>}{report.status === "complete" && <a href={`/api/reports/${report.id}/download`} className="mt-5 inline-flex items-center gap-2 text-sm font-semibold text-primary">Open private PDF <Download className="h-4 w-4" /></a>}</article>)}</div> : <div className="mt-5 rounded-2xl border border-dashed border-white/15 p-8 text-sm text-muted-foreground">No reports yet. Start with the free website readiness snapshot.</div>}</section>

    <section className="mt-16"><span className="eyebrow mb-4">Advanced Reports</span><div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-end"><div><h2 className="font-heading text-3xl font-bold">Reports and bundles</h2><p className="mt-2 max-w-2xl text-sm leading-6 text-muted-foreground">Move beyond a public-page snapshot with market, search, local, paid-media, reputation and conversion evidence. Comprehensive SEO and PPC reports include a 30 / 60 / 90 / 120-day action plan.</p></div><ShoppingBag className="h-8 w-8 text-primary" /></div><div className="mt-5 grid gap-4 md:grid-cols-2 lg:grid-cols-3">{Object.values(REPORT_OFFERS).map((offer) => <article key={offer.code} className="rounded-2xl border border-white/10 bg-card p-6"><h3 className="font-semibold">{offer.name}</h3><div className="mt-3 font-heading text-3xl font-bold text-primary">${(offer.amountCents / 100).toFixed(0)}</div><p className="mt-2 text-xs leading-5 text-muted-foreground">{offer.summary || "Choose the report areas that matter most to this business."}</p><p className="mt-3 text-xs leading-5 text-white/45">Hosting credit: {offer.hostingCredit}</p><DashboardCheckout offerCode={offer.code} /></article>)}</div><div className="mt-6 rounded-xl border border-white/10 bg-white/[0.025] p-4 text-xs leading-5 text-muted-foreground">{REPORTING_POLICY.changeNotice} Eligible managed service packages also include recurring website report cards so customers can monitor changes over time.</div><Link href="/systems/sis" className="mt-6 inline-flex items-center gap-2 text-sm font-semibold text-primary">Compare all report contents <ArrowRight className="h-4 w-4" /></Link></section>
